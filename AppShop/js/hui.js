...
function showProductInfo(pid){
	hui.open('productinfo.html',{},true,{pid:pid});
}
...
function payOrder(oid){
    //微信支付步骤
    //准备工作
    //注册微信开发 https://open.weixin.qq.com 并认证
    //填写应用信息 Android平台
    //应用下载地址：未填写
    //应用签名：59201cf6589202cb2cdab26752472112
    //http://ask.dcloud.net.cn/article/68 获取签名
    //包名：点击发行原生安装包即可查看或修改
    //注意以上信息一定要对称
    //开通微信支付
    //以上要进行2次认证,每次300元
    //1. 获取支付通道
    var channels, wxpay = null;
    plus.payment.getChannels(function(s){
        channels = s;
        for(var k in  channels){
            if(channels[k].id == 'wxpay'){wxpay = channels[k];}
        }
        //2. 判断是否具备微信支付环境
        if(wxpay == null){hui.toast("微信支付启动失败"); return false;}
        //3. 进行微信支付
        wxpaynow(oid, wxpay);
    }, function(e){
        hui.toast("获取支付通道失");
...
function wxpaynow(oid, wxpay){
    //3.1 获取服务器端的订单交易回话标识(符合微信支付要求的格式)
    //微信支付文档
    ...
    hui.getJSON(
        'http://shop.hcoder.net/pay.php?oid='+oid+'&suid='+SUID+'&srand='+SRAND,
        function(res){
            //console.log(res);
            if(!res.hasOwnProperty('appid')){hui.toast(" 服务器忙，请稍候重试！"); return false;}
            plus.payment.request(wxpay, JSON.stringify(res), function(result){
                    hui.toast('支付成功');
                },function(e){hui.toast(' 支付失败或被取消 ');}
            );
        }
    );
    //注意需要打包才能生效!!
